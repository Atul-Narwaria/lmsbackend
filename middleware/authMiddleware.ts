...
import { CheckSalesTeamExistance } from '../model/sales_team/salesTeam';
...
export const isSalesTeam = async (req: Request, res: Response, next: NextFunction) => {
    let id:string = req.userid;
    console.log(req.userid)
    if(!id){
       return res.status(401).json({ message: 'Invalid token' });
    }
        const check = await CheckSalesTeamExistance(id)
        console.log(check)
        if(check == 0){
